...
contract MultiCollateralHintHelpers is DynamoBase {
    IBorrowerOperations public immutable borrowerOperations;

    constructor(address _borrowerOperationsAddress, uint256 _gasCompensation) DynamoBase(_gasCompensation) {
        ...
        while (currentTroveuser != address(0) && remainingDebt > 0 && _maxIterations-- > 0) {
            ...
            if (netDebt > remainingDebt) {
                if (netDebt > minNetDebt) {
                    uint256 maxRedeemableDebt = DynamoMath._min(remainingDebt, netDebt - minNetDebt);
                    ...
                    partialRedemptionHintNICR = DynamoMath._computeNominalCR(newColl, compositeDebt);
    ...
    */
    function getApproxHint(
        ...
        diff = DynamoMath._getAbsoluteDifference(_CR, troveManager.getNominalICR(hintAddress));
        ...
        while (i < _numTrials) {
            ...
            uint256 currentDiff = DynamoMath._getAbsoluteDifference(currentNICR, _CR);
    ...
    function computeNominalCR(uint256 _coll, uint256 _debt) external pure returns (uint256) {
        return DynamoMath._computeNominalCR(_coll, _debt);
    }

    function computeCR(uint256 _coll, uint256 _debt, uint256 _price) external pure returns (uint256) {
        return DynamoMath._computeCR(_coll, _debt, _price);
    }
}
