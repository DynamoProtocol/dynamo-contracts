...
/**
    @title Dynamo Multi Token Price Feed
    @notice Based on Gravita's PriceFeed:
            ...
            Dynamo's implementation additionally caches price values within a block and incorporates exchange rate settings for derivative tokens (e.g. stETH -> wstETH).
 */
contract PriceFeed is DynamoOwnable {
    struct OracleRecord {
    ...
    constructor(address _dynamoCore, address ethFeed, OracleSetup[] memory oracles) DynamoOwnable(_dynamoCore) {
    ...
    function _isPriceChangeAboveMaxDeviation(
        ...
        uint256 minPrice = DynamoMath._min(currentScaledPrice, prevScaledPrice);
        uint256 maxPrice = DynamoMath._max(currentScaledPrice, prevScaledPrice);
         ...
         */
        uint256 percentDeviation = ((maxPrice - minPrice) * DynamoMath.DECIMAL_PRECISION) / maxPrice;
