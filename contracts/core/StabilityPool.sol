...
import "../dependencies/DynamoMath.sol";
import "../interfaces/IDebtToken.sol";
import "../interfaces/IVault.sol";

/**
    @title Dynamo Stability Pool
    @notice Based on Liquity's `StabilityPool`
            ...
            Dynamo's implementation is modified to support multiple collaterals. Deposits into
            the stability pool may be used to liquidate any supported collateral type.
 */
contract StabilityPool is DynamoOwnable, SystemStart {
    ...
    IDynamoVault public immutable vault;
    ...
    /*
     * Similarly, the sum 'G' is used to calculate Dynamo gains. During it's lifetime, each deposit d_t earns a Dynamo gain of
     *  ( d_t * [G - G_t] )/P_t, where G_t is the depositor's snapshot of G taken at time t when  the deposit was made.
     *
     *  Dynamo reward events occur are triggered by depositor operations (new deposit, topup, withdrawal), and liquidations.
     *  In each case, the Dynamo reward is issued (i.e. G is updated), before other state changes are made.
     */
    mapping(uint128 => mapping(uint128 => uint256)) public epochToScaleToG;

    // Error tracker for the error correction in the Dynamo issuance calculation
    uint256 public lastDynamoError;
    ...
    ) DynamoOwnable(_dynamoCore) SystemStart(_dynamoCore) {
    ...
    /*  provideToSP():
     *
     * - Triggers a Dynamo issuance, based on time passed since the last issuance. The Dynamo issuance is shared between *all* depositors and front ends
     * - Tags the deposit with the provided front end tag param, if it's a new deposit
     * - Sends depositor's accumulated gains (Dynamo, collateral) to depositor
     * - Sends the tagged front end's accumulated Dynamo gains to the tagged front end
     * - Increases deposit and tagged front end's stake, and takes new snapshots for each.
     */
    function provideToSP(uint256 _amount) external {
        require(!DYNAMO_CORE.paused(), "Deposits are paused");
     ...
     * - Sends all depositor's accumulated gains (Dynamo, collateral) to depositor
     * - Sends the tagged front end's accumulated Dynamo gains to the tagged front end
    ...
    function withdrawFromSP(uint256 _amount) external {
        ...
        uint256 compoundedDebtDeposit = getCompoundedDebtDeposit(msg.sender);
        uint256 debtToWithdraw = DynamoMath._min(_amount, compoundedDebtDeposit);
    ...
    function _updateG(uint256 _dynamoIssuance) internal {
        uint256 totalDebt = totalDebtTokenDeposits; // cached to save an SLOAD
        /*
         * When total deposits is 0, G is not updated. In this case, the Dynamo issued can not be obtained by later
         * depositors - it is missed out on, and remains in the balanceof the Treasury contract.
         *
         */
        if (totalDebt == 0 || _dynamoIssuance == 0) {
            return;
        }

        uint256 dynamoPerUnitStaked;
        dynamoPerUnitStaked = _computeDynamoPerUnitStaked(_dynamoIssuance, totalDebt);
        uint128 currentEpochCached = currentEpoch;
        uint128 currentScaleCached = currentScale;
        uint256 marginalDynamoGain = dynamoPerUnitStaked * P;
        uint256 newG = epochToScaleToG[currentEpochCached][currentScaleCached] + marginalDynamoGain;
        epochToScaleToG[currentEpochCached][currentScaleCached] = newG;

        emit G_Updated(newG, currentEpochCached, currentScaleCached);
    }

    function _computeDynamoPerUnitStaked(
        uint256 _dynamoIssuance,
        uint256 _totalDebtTokenDeposits
    ) internal returns (uint256) {
        /*
         * Calculate the Dynamo-per-unit staked.  Division uses a "feedback" error correction, to keep the
         ...
         */
        uint256 dynamoNumerator = (_dynamoIssuance * DECIMAL_PRECISION) + lastDynamoError;

        uint256 dynamoPerUnitStaked = dynamoNumerator / _totalDebtTokenDeposits;
        lastDynamoError = dynamoNumerator - (dynamoPerUnitStaked * _totalDebtTokenDeposits);

        return dynamoPerUnitStaked;
    }
    ...
    /*
     * Calculate the Dynamo gain earned by a deposit since its last snapshots were taken.
     * Given by the formula:  Dynamo = d0 * (G - G(0))/P(0)
    ...
    function claimableReward(address _depositor) external view returns (uint256) {
        ...
        uint256 dynamoNumerator = (_vestedEmissions() * DECIMAL_PRECISION) + lastDynamoError;
        uint256 dynamoPerUnitStaked = dynamoNumerator / totalDebt;
        uint256 marginalDynamoGain = dynamoPerUnitStaked * P;
        ...
        if (scaleSnapshot == currentScale) {
            firstPortion = epochToScaleToG[epochSnapshot][scaleSnapshot] - snapshots.G + marginalDynamoGain;
            ...
            secondPortion = (epochToScaleToG[epochSnapshot][scaleSnapshot + 1] + marginalDynamoGain) / SCALE_FACTOR;
        }
    ...
    function _claimableReward(address _depositor) private view returns (uint256) {
        ...
        return _getDynamoGainFromSnapshots(initialDeposit, snapshots);
    }

    function _getDynamoGainFromSnapshots(
        uint256 initialStake,
        Snapshots memory snapshots
    ) internal view returns (uint256) {
        /*
         * Grab the sum 'G' from the epoch at which the stake was made. The Dynamo gain may span up to one scale change.
         * If it does, the second portion of the Dynamo gain is scaled by 1e9.
        ...
        uint256 dynamoGain = (initialStake * (firstPortion + secondPortion)) / P_Snapshot / DECIMAL_PRECISION;
    ...
    // --- Sender functions for Debt deposit, collateral gains and Dynamo gains ---
