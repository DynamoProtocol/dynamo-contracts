...
import "../dependencies/DynamoBase.sol";
import "../dependencies/DynamoMath.sol";
import "../dependencies/DynamoOwnable.sol";

/**
    @title Dynamo Trove Manager
    @notice Based on Liquity's `TroveManager`
            https://github.com/liquity/dev/blob/main/packages/contracts/contracts/TroveManager.sol

            Dynamo's implementation is modified so that multiple `TroveManager` and `SortedTroves`
 ...
 */
contract TroveManager is DynamoBase, DynamoOwnable, SystemStart {
    ...
    ) DynamoOwnable(_dynamoCore) DynamoBase(_gasCompensation) SystemStart(_dynamoCore) {
        gasPoolAddress = _gasPoolAddress;
        debtToken = IDebtToken(_debtTokenAddress);
        borrowerOperationsAddress = _borrowerOperationsAddress;
        vault = IDynamoVault(_vault);
    ...
    function collectInterests() external {
        ...
        debtToken.mint(DYNAMO_CORE.feeReceiver(), interestPayableCached);
        interestPayable = 0;
    }
    ...
    function fetchPrice() public returns (uint256) {
        ...
        if (address(_priceFeed) == address(0)) {
            _priceFeed = IPriceFeed(DYNAMO_CORE.priceFeed());
        }
    ...
    function getNominalICR(address _borrower) public view returns (uint256) {
        ...
        uint256 NICR = DynamoMath._computeNominalCR(currentCollateral, currentDebt);
        return NICR;
    }
    ...
    function getCurrentICR(address _borrower, uint256 _price) public view returns (uint256) {
        ...
        uint256 ICR = DynamoMath._computeCR(currentCollateral, currentDebt, _price);
        return ICR;
    }
    ...
    function _updateBaseRateFromRedemption(
        ...
        newBaseRate = DynamoMath._min(newBaseRate, DECIMAL_PRECISION); // cap baseRate at a maximum of 100%
    ...
    function _calcBorrowingRate(uint256 _baseRate) internal view returns (uint256) {
        return DynamoMath._min(borrowingFeeFloor + _baseRate, maxBorrowingFee);
    }
    ...
    function _calcDecayedBaseRate() internal view returns (uint256) {
        ...
        uint256 decayFactor = DynamoMath._decPow(minuteDecayFactor, minutesPassed);
    ...
    function redeemCollateral(
        ...
        _sendCollateral(DYNAMO_CORE.feeReceiver(), totals.collateralFee);
    ...
    function _redeemCollateralFromTrove(
        ...
        singleRedemption.debtLot = DynamoMath._min(_maxDebtAmount, t.debt - DEBT_GAS_COMPENSATION);
        ...
        if (newDebt == DEBT_GAS_COMPENSATION) {
            ...
            uint256 newNICR = DynamoMath._computeNominalCR(newColl, newDebt);
            /*
