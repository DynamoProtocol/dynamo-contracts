...
/**
    @notice Dynamo Curve Factory
    @title Deploys clones of `CurveDepositToken` as directed by the Dynamo DAO
 */
contract CurveFactory is DynamoOwnable {
    ...
    /**
        @dev After calling this function, the owner should also call `Vault.registerReceiver`
             to enable DYNAMO emissions on the newly deployed `CurveDepositToken`
     */
