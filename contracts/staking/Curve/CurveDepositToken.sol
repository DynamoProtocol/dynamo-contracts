...
/**
    @title Dynamo Curve Deposit Wrapper
    @notice Standard ERC20 interface around a deposit of a Curve LP token into it's
            ...
            burned to receive the LP tokens back. Holders may claim DYNAMO emissions
            on top of the earned CRV.
 */
contract CurveDepositToken is DynamoOwnable {
    IERC20 public immutable DYNAMO;
    ...
    // each array relates to [DYNAMO, CRV]
    ...
    event RewardClaimed(address indexed receiver, uint256 dynamoAmount, uint256 crvAmount);
    ...
    function initialize(ILiquidityGauge _gauge) external {
        ...
        DYNAMO.approve(address(vault), type(uint256).max);

        string memory _symbol = IERC20Metadata(_token).symbol();
        name = string.concat("Dynamo ", _symbol, " Curve Deposit");
        symbol = string.concat("dynamo-", _symbol);
    ...
    function claimReward(address receiver) external returns (uint256 dynamoAmount, uint256 crvAmount) {
    ...
    function claimableReward(address account) external view returns (uint256 dynamoAmount, uint256 crvAmount) {
    ...
    function _pushExcessEmissions(uint256 newAmount) internal {
        if (vault.lockWeeks() > 0) storedExcessEmissions = uint128(storedExcessEmissions + newAmount);
            ...
            vault.increaseUnallocatedSupply(DYNAMO.balanceOf(address(this)));
        }
    }
    ...
    function _fetchRewards() internal {
        uint256 dynamoAmount;
        uint256 id = emissionId;
        if (id > 0) dynamoAmount = vault.allocateNewEmissions(id);
        ...
        if (maxWeekly < 10000) {
            ...
            if (dynamoAmount > maxWeekly) {
                emit MaxWeeklyEmissionsExceeded(dynamoAmount, maxWeekly);
                _pushExcessEmissions(dynamoAmount - maxWeekly);
                dynamoAmount = maxWeekly;
            }
        }
        ...
        if (block.timestamp < _periodFinish) {
            ...
            dynamoAmount += remaining * rewardRate[0];
            crvAmount += remaining * rewardRate[1];
        }
        rewardRate[0] = uint128(dynamoAmount / REWARD_DURATION);
