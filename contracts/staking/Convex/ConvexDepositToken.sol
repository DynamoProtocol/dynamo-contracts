...
/**
    @title Dynamo Convex Deposit Wrapper
    @notice Standard ERC20 interface around a deposit of a Curve LP token into Convex.
            ...
            tokens back. Holders may claim DYNAMO emissions on top of the earned CRV and CVX.
 */
contract ConvexDepositToken is DynamoOwnable {
    ...
    // each array relates to [DYNAMO, CRV, CVX]
    ...
    event RewardClaimed(address indexed receiver, uint256 dynamoAmount, uint256 crvAmount, uint256 cvxAmount);
    ...
    function initialize(uint256 pid) external {
        ...
        name = string.concat("Dynamo ", _symbol, " Convex Deposit");
    ...
    function claimReward(
        address receiver
    ) external returns (uint256 dynamoAmount, uint256 crvAmount, uint256 cvxAmount) {
    ...
    function claimableReward(
        address account
    ) external view returns (uint256 dynamoAmount, uint256 crvAmount, uint256 cvxAmount) {
