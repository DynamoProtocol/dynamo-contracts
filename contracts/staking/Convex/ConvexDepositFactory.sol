...
/**
    @notice Dynamo Convex Factory
    @title Deploys clones of `ConvexDepositToken` as directed by the Dynamo DAO
 */
contract ConvexFactory is DynamoOwnable {
    ...
    /**
        @dev After calling this function, the owner should also call `Vault.registerReceiver`
             to enable DYNAMO emissions on the newly deployed `ConvexDepositToken`
     */
