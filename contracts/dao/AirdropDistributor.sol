...
/**
    @title Dynamo Airdrop Distributor
    @notice Distributes DYNAMO to veCRV holders that voted in favor of Dynamo's
            initial Curve governance proposal, and to early users who interacted
            with the protocol prior to token emissions.
    @dev Airdropped DYNAMO tokens are given as a locked position. Distribution
         is via a merkle proof. The proof and script used to create are available
         on Github: https://github.com/dynamo-fi/airdrop-proofs
 */
contract AirdropDistributor is Ownable {
    ...
    function sweepUnclaimedTokens() external {
        ...
        IDynamoVault(vault).increaseUnallocatedSupply(amount);
    }
