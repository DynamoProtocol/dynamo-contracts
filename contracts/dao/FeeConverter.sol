...
contract AddLiquidityChecker is DynamoOwnable {
    ...
    constructor(address _core, ICryptoSwap _curve, uint256 _maxDeviation) DynamoOwnable(_core) {
        curvePool = _curve;
...
contract FeeConverter is DynamoOwnable, SystemStart {
    using SafeERC20 for IERC20;

    IFeeDistributor public immutable feeDistributor;
    IERC20 public immutable debtToken;
    IERC20 public immutable dynamoToken;
    ...
    event LiquidityAdded(uint256 priceScale, uint256 debtAmount, uint256 dynamoAmount, uint256 lpAmountReceived);
    ...
    constructor(
        ...
        _dynamoToken.approve(address(_curvePool), type(uint256).max);
    ...
    function setIsSellingCollateral(address[] memory collaterals, bool isSelling) public onlyOwner {
        ...
        if (isSelling) {
            IPriceFeed feed = IPriceFeed(DYNAMO_CORE.priceFeed());
    ...
    function swapDebtForColl(address collateral, uint256 debtAmount) external returns (uint256) {
        ...
        address receiver = DYNAMO_CORE.feeReceiver();
    ...
    function addPendingLiquidity() external returns (bool) {
        ...
        if (amount > 0) {
            ...
            uint added = _addLiquidity(amount, DYNAMO_CORE.feeReceiver());
    ...
    function recoverToken(IERC20 token) external onlyOwner returns (bool) {
        ...
        if (amount > 0) {
            ...
            token.safeTransfer(DYNAMO_CORE.feeReceiver(), amount);
        }
        return true;
    }
    ...
    function _addLiquidity(uint256 debtAmount, address receiver) internal returns (uint256) {
        ...
        uint256 dynamoAmount = (debtAmount * 1e18) / priceScale;
        uint256 dynamoAvailable = dynamoToken.balanceOf(receiver);

        // if insufficient DYNAMO is available, adjust the amounts
        if (dynamoAvailable < dynamoAmount) {
            if (dynamoAvailable < 1e18) return 0;
            dynamoAmount = dynamoAvailable;
            debtAmount = (dynamoAmount * priceScale) / 1e18;
        }

        dynamoToken.transferFrom(receiver, address(this), dynamoAmount);
        uint256 lpAmount = curvePool.add_liquidity([debtAmount, dynamoAmount], 0, false, receiver);

        emit LiquidityAdded(priceScale, debtAmount, dynamoAmount, lpAmount);
        return debtAmount;
    }
}
