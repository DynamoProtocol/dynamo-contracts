...
import "../interfaces/IDynamoToken.sol";
...
/**
    @title Dynamo Vault
    @notice The total supply of DYNAMO is initially minted to this contract.
 ...
 */
contract DynamoVault is DynamoOwnable, SystemStart {
    using Address for address;
    using SafeERC20 for IERC20;

    IDynamoToken public immutable dynamoToken;
    ...
    // `dynamoToken` balance within the treasury that is not yet allocated.
    // Starts as `dynamoToken.totalSupply()` and decreases over time.
    ...
    // number of weeks that DYNAMO is locked for when transferred using
    ...
    // account -> week -> DYNAMO amount claimed in that week (used for calculating boost)
    ...
    function setInitialParameters(
        ...
        dynamoToken.mintToVault(totalSupply);
        ...
        for (uint256 i = 0; i < length; i++) {
            ...
            dynamoToken.increaseAllowance(receiver, amount);
        }
    ...
    function transferTokens(IERC20 token, address receiver, uint256 amount) external onlyOwner returns (bool) {
        if (address(token) == address(dynamoToken)) {
    ...
    /**
        @notice Receive DYNAMO tokens and add them to the unallocated supply
     */
    function increaseUnallocatedSupply(uint256 amount) external returns (bool) {
        dynamoToken.transferFrom(msg.sender, address(this), amount);
        uint256 unallocated = unallocatedTotal + amount;
        unallocatedTotal = uint128(unallocated);
    ...
    /**
        @notice Allocate additional `dynamoToken` allowance to an emission reciever
                based on the emission schedule
        @param id Receiver ID. The caller must be the receiver mapped to this ID.
        @return uint256 Additional `dynamoToken` allowance for the receiver. The receiver
    ...
    /**
        @notice Transfer `dynamoToken` tokens previously allocated to the caller
    ...
    function _transferOrLock(address claimant, address receiver, uint256 amount) internal {
        ...
        if (_lockWeeks == 0) {
            ...
            dynamoToken.transfer(receiver, amount);
        } else {
    ...
    /**
        @notice Claimable DYNAMO amount for `account` in `rewardContract` after applying boost
