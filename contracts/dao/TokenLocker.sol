...
/**
    @title Dynamo Token Locker
    @notice DYNAMO tokens can be locked in this contract to receive "lock weight",
            which is used within `AdminVoting` and `IncentiveVoting` to vote on
            core protocol operations.
 */
contract TokenLocker is DynamoOwnable, SystemStart {
    ...
    IDynamoToken public immutable lockToken;
    ...
    ) SystemStart(_dynamoCore) DynamoOwnable(_dynamoCore) {
        lockToken = _token;
    ...
    function withdrawWithPenalty(
        ...
        lockToken.transfer(msg.sender, amountToWithdraw);
        lockToken.transfer(dynamoCore.feeReceiver(), penaltyTotal);
