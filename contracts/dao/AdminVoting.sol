...
/**
    @title Dynamo DAO Admin Voter
    @notice Primary ownership contract for all Dynamo contracts. Allows executing
            arbitrary function calls only after a required percentage of DYNAMO
...
contract AdminVoting is DelegatedOps, SystemStart {
    ...
    IDynamoCore public immutable dynamoCore;
    ...
    ) SystemStart(_dynamoCore) {
        tokenLocker = _tokenLocker;
        dynamoCore = IDynamoCore(_dynamoCore);
    ...
    function createNewProposal(address account, Action[] calldata payload) external callerOrDelegated(account) {
        ...
        // if the only action is `dynamoCore.setGuardian()`, use
    ...
    function cancelProposal(uint256 id) external {
        require(msg.sender == dynamoCore.guardian(), "Only guardian can cancel proposals");
        require(id < proposalData.length, "Invalid ID");
    ...
    /**
        @dev Unguarded method to allow accepting ownership transfer of `DynamoCore`
             at the end of the deployment sequence
     */
    function acceptTransferOwnership() external {
        dynamoCore.acceptTransferOwnership();
    }

    function _isSetGuardianPayload(uint256 payloadLength, Action memory action) internal view returns (bool) {
        if (payloadLength == 1 && action.target == address(dynamoCore)) {
            ...
            return sig == IDynamoCore.setGuardian.selector;
        }
        return false;
    }
}
