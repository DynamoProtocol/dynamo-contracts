...
/**
    @title Dynamo DAO Interim Admin
    @notice Temporary ownership contract for all Dynamo contracts during bootstrap phase. Allows executing
...
contract InterimAdmin is Ownable {
    ...
    function cancelProposal(uint256 id) external {
        require(msg.sender == owner() || msg.sender == dynamoCore.guardian(), "Unauthorized");
    ...
    /**
        @dev Allow accepting ownership transfer of `DynamoCore`
     */
    ...
    /**
        @dev Restricted method to transfer ownership of `DynamoCore`
             to the actual Admin voting contract
     */
    function transferOwnershipToAdminVoting() external {
        require(msg.sender == owner() || msg.sender == dynamoCore.guardian(), "Unauthorized");
        dynamoCore.commitTransferOwnership(adminVoting);
    }
